...
- (void)setParameterWithData:(int)param data:(NSData *)data {
    ...
    if (data.length == 8192) {
        ...
        signed char *arr = const_cast<signed char *>(reinterpret_cast<const signed char *>(bytes + sizeof(int) + sizeof(uint32_t)));
        ...
        signed char *arr = const_cast<signed char *>(reinterpret_cast<const signed char *>(bytes + sizeof(uint32_t)));
        _engine.DispatchCommand(param, 0, 0, 0, 0, arrSize, arr);
    } else {
        _engine.DispatchCommand(param, 0, 0, 0, 0,
